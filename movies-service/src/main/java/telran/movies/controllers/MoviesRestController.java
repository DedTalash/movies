...
public class MoviesRestController {
    ...
    MovieDto movieName(@RequestParam String name){
	    return service.getMovie(name);
    }

	@GetMapping("movies/watchesCount")
	List<MovieDto> movieName(@RequestParam int count){
		return service.moviesWatchesGreaterThan(count);
	}

	@GetMapping("movies/filter/year")
	List<MovieDto> filterYear(
		...
		return service.getMoviesYears(from, to);
	}

	@GetMapping("movies/random")
	List<WatchDto> createRandomData(
		@RequestParam int nMovies,
		@RequestParam int nCinemas,
		@RequestParam int nWatches
	) {
	...
	private List<WatchDto> getRandomWatches(
		ArrayList<MovieDto> movies,
		ArrayList<CinemaDto> cinemas,
		int nWatches
	) {
		return Stream.generate(() -> getRandomWatch(movies, cinemas))
			.limit(nWatches)
			.collect(Collectors.toList());
	}

	private ArrayList<CinemaDto> getRandomCinemas(int nCinemas) {
		return Stream.generate(this::getRandomCinema).distinct()
			.limit(nCinemas)
